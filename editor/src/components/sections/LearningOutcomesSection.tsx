...
export function LearningOutcomesSection({ items, onChange, readOnly = false }: Props) {
    ...
    <div className="space-y-2">
      {items.map((item, i) => (
        <div key={i} className="flex items-center gap-2">
          ...
          <Input value={item} onChange={e => update(i, e.target.value)} placeholder={`Outcome ${i + 1}`} className="flex-1" />
